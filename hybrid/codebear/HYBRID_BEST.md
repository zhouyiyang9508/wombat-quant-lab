...
更新时间: 2026-02-22 (v6) | 作者: 代码熊 🐻

---

## 🏆 当前冠军：Hybrid v6 (v11b股票 + v5-D Crypto)

**文件**: `hybrid_v6_v11b_v5d.py`  
**架构**: 股票(v11b, 三层防御) + 加密(BTC/ETH 阈值切换+独立MA过滤, 修复前瞻偏差)

### 推荐配置（日频回测，IS=2015-2020，OOS=2021-2025）

| w_crypto | CAGR | MaxDD | Sharpe | WF | IS_Sh | OOS_Sh | 说明 |
|---|---|---|---|---|---|---|---|
| **15%** | **45.2%** | **-18.9%** | 1.80 | **0.644** ✅ | 2.15 | 1.38 | 🏆 最优风险调整 |
| **20%** | **48.6%** | **-20.4%** | 1.79 | **0.638** ✅ | 2.12 | 1.35 | 最高 CAGR (WF≥0.60) |
| 25% | 52.0% | -25.0% | 1.74 | 0.635 ✅ | 2.06 | 1.31 | 激进配置 |

> ✅ 所有数字均修复前瞻偏差（MA 过滤使用 prev_day 价格）  
> ✅ StockV11b 日频数据来自 hybrid_v4_equity.csv  
> ✅ Crypto v5-D 独立: CAGR=47.9%, MaxDD=-74.7%, Sharpe=0.92, WF=0.546

### 关键改进（vs 上代 v5-D + v9j）

| 指标 | v5-D+v9j (w=20%) | **v6 (w=15%)** | **v6 (w=20%)** |
|---|---|---|---|
| CAGR | 48.3% | 45.2% (-3.1%) | **48.6%** (+0.3%) |
| MaxDD | -22.1% | **-18.9%** (+3.2%) | **-20.4%** (+1.7%) |
| WF | 0.640 | **0.644** (+0.004) | 0.638 (-0.002) |

**v6 核心价值**: v11b 防御层把日频 MaxDD 从 -22.1% 压到 -18.9%（首次突破 20% 关口）

---

## 历史版本（已弃用）：Hybrid v5 策略D
...
| **v6 w=15%** | **v11b daily** | **BTC/ETH 独立MA+阈值(v5D)** | **45.2%** | **0.644** ✅ | **-18.9%** 🏆 |
| **v6 w=20%** | **v11b daily** | **BTC/ETH 独立MA+阈值(v5D)** | **48.6%** | **0.638** ✅ | **-20.4%** |

**关键教训:**
- 扩展 Crypto 池（v4 altcoins）适得其反 → BTC+ETH 就是最优 Crypto 组合
- 独立 MA 过滤 > 联动 MA 过滤（允许 ETH 单独持有）
- 阈值切换比连续权重 WF 更高（减少噪音信号）
- 更强的股票端防御（v11b vs v9j）→ MaxDD 从 -22.1% 降到 -18.9%（w=15%）
- 科技宽度替代全市场宽度（v12b 实验）：WF +0.051 但 CAGR -1.7%，净效益为负，不推进
