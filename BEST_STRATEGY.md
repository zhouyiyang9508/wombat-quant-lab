...
> Updated: 2026-02-22 by 代码熊 🐻 (v11b/v11c参数优化 + v11a日频审计)
...
> 
> ⭐ **策略选择指南**:
> - 🏆 **NEW CHAMP**: v11a (Composite **2.190**, WF 0.74, WF3=**0.82**) — 三大创新叠加，首破2.15！MaxDD首次<-9.5%! (CAGR 32.1%, Sharpe 1.92, Calmar 3.40)
> - 上代冠军: v10d (Composite 2.107, WF 0.77) — 日频审计通过 (<25%)
> - 最稳健(WF): v9j (2.057, WF 0.78) — 最高WF，保守选择
> - ⚠️ 日频MaxDD: v10d月-10%→日-23.8% (2.38x); v11a月-9.4%→日**-20.2%** (2.14x) 均<25%目标

| Rank | Strategy | CAGR | MaxDD | Sharpe | Calmar | Composite | WF | WF3 | Notes |
|------|----------|------|-------|--------|--------|-----------|-----|-----|-------|
| 🥇1 | **Stock v11a🆕 MasterStack** 🏆 | 32.1% | **-9.4%** | **1.92** | **3.40** | **2.190** | ✅ 0.74 | **0.82** | **首破2.15!MaxDD<10%!** |
| 2 | **Stock v10d🆕 RateAdaptHedge** | **32.5%** | -10.0% | 1.86 | 3.25 | **2.107** | ✅ 0.77 | — | 首破2.1 |
| 3 | **Stock v9n Combo** | 32.2% | -10.0% | 1.84 | 3.22 | **2.090** | ✅ 0.70 | — | |
| 4 | **Stock v9m SPY-Soft** | 32.0% | -10.0% | 1.84 | 3.21 | **2.086** | ✅ 0.75 | — | |
| 5 | **Stock v9l AdaptVol** | 32.1% | -10.0% | 1.83 | 3.20 | **2.077** | ✅ 0.71 | — | |
| 6 | **Stock v10c DefBridge+Hedge** | 31.7% | -10.2% | 1.91 | 3.11 | **2.072** | ✅ 0.75 | — | |
| 7 | **Stock v10b DefensiveBridge** | 31.6% | -10.2% | 1.90 | 3.10 | **2.064** | ✅ 0.76 | — | |
| 8 | **Stock v9j TLT Bear** ⭐稳健 | 32.3% | -10.3% | 1.85 | 3.13 | **2.057** | ✅ **0.78** | — | 最高WF |
| 9 | Stock v9i VolTarget-11% | 31.9% | -10.7% | 1.81 | 2.97 | 1.973 | ✅ 0.82 | — | |
...
## Round 6 (2026-02-22): v11b行业ETF失败 / v11c参数验证 / v11a日频审计

### v11b — 行业ETF趋势过滤器 ❌ 失败
**File**: `stocks/codebear/momentum_v11b.py`

行业ETF 6m动量过滤: 只进入ETF动量>0的行业 → 结果相反!
- 最优thresh=-0.05: Comp=2.083 (vs v11a 2.190, -0.107!)
- WF=0.65 (vs v11a 0.74, -0.09!)
- 结论: 个股动量聚合是比ETF动量更好的行业选择信号

### v11c — 参数联合扫描 (CONT_BONUS × DEFENSIVE_FRAC × SPY_SOFT)
**File**: `stocks/codebear/momentum_v11c_sweep.py`

v11a参数基本已接近最优, 扫描发现:
- CONT_BONUS: 0.03 是最优 (增大会破坏选股质量)
- DEFENSIVE_FRAC: **12%** 优于 15% (Comp 2.191 vs 2.160)
- SPY_SOFT_HI_FRAC: 0.10~0.12 均优 (差异极小)
- 最优配置: `cb=0.03, def=0.12, sf=0.12` → Comp **2.1916**, WF 0.74

### v11a 日频审计 (最重要的风险验证)
- 月频MaxDD: -9.44% → 日频真实MaxDD: **-20.24%** (2.14x低估比)
- 峰值: 2020-02-20 → 谷底: 2020-03-18 (COVID月内暴跌)
- 日频Sharpe: 1.95, CAGR: 32.2%
- **< 25%目标 ✅ PASS** (优于v10d的-23.81%)
- 三大叠加的防御机制确实减少了日频最大回撤 (-3.57% vs v10d)

---

## 🚨🚨 Latest Exploration (2026-02-22, Round 5): v11a/v10d-Audit — 【重大突破×2】

### Stock v11a Master — 三大创新叠加 🏆 Composite 2.160! MaxDD -9.5%!
**File**: `stocks/codebear/momentum_v11a_master.py`

**三大正交创新组合** (无相互干扰):
1. v10d: 熊市时IEF/TLT动量自适应选择 (双负→不用债券)
2. v10b: 软牛市(45-65% breadth)加入15% XLV/XLP/XLU防御行业
3. v9m: SPY月跌>7%时预先+10% GLD软对冲

**里程碑记录**:
| 指标 | v9j(基准) | v10d | v11a | 记录 |
|------|-----------|------|------|------|
| Composite | 2.057 | 2.107 | **2.190** | 历史最高 |
| MaxDD | -10.3% | -10.0% | **-9.5%** | 首次<-10% |
| Calmar | 3.13 | 3.25 | **3.33** | 历史最高 |
| Sharpe | 1.85 | 1.86 | **1.91** | 平历史最高 |
| WF3 | — | — | **0.82** | 3期验证强健 |
| WF (2-期) | 0.78 | 0.77 | 0.74 | -0.04 |

**3-期Walk Forward验证** (更严格):
- IS (2015-2018): Sharpe 1.86
- OOS1 (2019-2021): Sharpe 2.44 (超越IS!)
- OOS2 (2022-2025): Sharpe 1.52 (加息年仍强劲)
- WF3 = min(2.44, 1.52) / 1.86 = **0.82** ← 极强样本外一致性

### v10d 日频审计
**File**: `stocks/codebear/daily_audit_v10d.py`

- 月频报告MaxDD: -10.0%
- 日频真实MaxDD: **-23.81%** (< 25%目标 ✅)
- 低估倍数: 2.38x (高于v9f/v9g的1.78x)
- 最大回撤期: 2020-02-20 → 2020-03-20 (COVID月内暴跌)
- 日频Sharpe: 1.91, 日频CAGR: 33.2%
- 推断v11a日频MaxDD: 约-9.5% × 2.38 = **≈-22.6%** (< 25% ✅)
