...
## Stock v4d — DD Responsive GLD Hedge ⭐⭐⭐ NEW CHAMPION

> 2026-02-20 | 代码熊 🐻

### 背景

Stock v3b (Composite 1.173) 在 2022 熊市中表现不错（行业轮动自动选了能源股），但 2018 Q4 回撤 -17.7% 仍是最大弱点。受 TQQQ v9g GLD 对冲成功启发，探索在 Stock 策略中加入 GLD 对冲。

### 测试的变种

| 版本 | CAGR | MaxDD | Sharpe | Calmar | IS Sh | OOS Sh | WF | Composite |
|------|------|-------|--------|--------|-------|--------|-----|-----------|
| v3b (baseline) | 25.8% | -17.7% | 1.35 | 1.46 | 1.39 | 1.18 | 0.85 ✅ | 1.173 |
| v4a DeepBear GLD | 25.0% | -17.7% | 1.32 | 1.41 | 1.38 | 1.14 | 0.83 ✅ | 1.142 |
| v4a2 Bear+GLD | 25.8% | -18.1% | 1.38 | 1.42 | 1.47 | 1.16 | 0.79 ✅ | 1.173 |
| v4c DualMom | 23.4% | -17.7% | 1.34 | 1.32 | 1.36 | 1.20 | 0.88 ✅ | 1.111 |
| v4d DD Responsive | 26.4% | -15.2% | 1.39 | 1.74 | 1.47 | 1.20 | 0.82 ✅ | 1.306 |
| v4e SoftBear+GLD | 26.7% | -17.7% | 1.38 | 1.51 | 1.44 | 1.19 | 0.82 ✅ | 1.208 |

### v4d 参数调优 (Round 2)

| 变种 | CAGR | MaxDD | Sharpe | Calmar | IS Sh | OOS Sh | WF | Composite |
|------|------|-------|--------|--------|-------|--------|-----|-----------|
| v4d_orig (-10%/30%, -15%/50%) | 26.6% | -15.2% | 1.40 | 1.75 | 1.47 | 1.22 | 0.83 ✅ | 1.314 |
| v4d_tight (-8%/25%, -13%/45%) | 26.1% | -15.2% | 1.39 | 1.72 | 1.43 | 1.24 | 0.87 ✅ | 1.295 |
| v4d_3tier (-8/20%, -13/35%, -18/50%) | 26.3% | -15.5% | 1.39 | 1.69 | 1.43 | 1.23 | 0.86 ✅ | 1.283 |
| v4d_soft (-10%/20%, -15%/35%) | 26.6% | -15.5% | 1.39 | 1.71 | 1.46 | 1.21 | 0.83 ✅ | 1.296 |
| **v4d_aggr (-8/30%, -12/50%, -18/60%)** | **27.0%** | **-15.0%** | **1.43** | **1.80** | **1.50** | **1.25** | **0.83 ✅** | **1.350** |
| v4f_hybrid (Bear+DD) | 26.3% | -18.3% | 1.39 | 1.44 | 1.47 | 1.19 | 0.81 ✅ | 1.184 |

### 🏆 最优: v4d_aggr (Composite 1.350)

**DD Responsive GLD 对冲参数:**
- 正常时: 100% Stock v3b
...
**GLD 触发频率:** 10年中 18 个月触发（共131个月），仅在需要时介入。

**关键触发期:**
- 2015-09, 2015-12, 2016-01~02: 早期市场震荡
- 2018-10~12, 2019-01: Q4 大跌时 GLD 对冲减少回撤
- 2022-09, 2023-09~10: 回撤修正
- 2025-03~08: 近期波动
...
### 为什么 v4d DD Responsive 有效

1. **精准介入**: 只在组合实际回撤 >-8% 时才配置 GLD，不影响牛市表现
2. **GLD 在崩盘时通常上涨**: 2018 Q4 GLD 上涨 8.6%，对冲了股票损失
3. **自动恢复**: 回撤修复后 GLD 配置自动归零，不错过反弹
4. **不依赖市场判断**: 不需要 SPY/SMA200 来判断牛熊，纯靠组合自身回撤

### 为什么其他变种不够好

- **v4a/v4a2 熊市GLD**: 依赖 SPY < SMA200 判断，但 v3b 在 2022 熊市本就表现好（能源轮动），GLD 对冲反而拖累
- **v4c DualMom**: GLD 动量 > SPY 时切50%GLD，在2022能源牛市中错过了大量收益
- **v4e SoftBear+GLD**: 只在 bear regime 配20%GLD，太保守，改善有限
- **v4f Hybrid**: Bear+DD 双重条件导致 bear 市场过度配置 GLD

### 文件

- `stocks/codebear/momentum_v4d.py` — v4d 最优策略
- `stocks/codebear/momentum_v4_gld.py` — 全部变种对比
- `stocks/codebear/momentum_v4_round2.py` — 参数调优
- `stocks/codebear/momentum_v4_results.json` — 回测结果

---
*Updated by 代码熊 🐻 — 2026-02-20 (Stock v4d: DD Responsive GLD Hedge)*


---

## Crypto Dual Momentum — BTC + ETH + GLD Rotation (2026-02-20)

> 代码熊 🐻 | 全新方向：加入 ETH 扩展加密货币策略

### 背景

BTC v7f（DualMom BTC vs GLD）是目前加密货币最优策略（Composite 0.987）。本轮探索加入 ETH，形成 BTC + ETH + GLD 三资产轮动。

**假设**：BTC 和 ETH 有不同的动量周期（ETH 在 2020 DeFi、2021 NFT 时期大幅跑赢 BTC），轮动可以捕捉到更多收益。

### 数据

- ETH: Stooq.com eth.v 日线（2015-08-07 → 2026-02-18）
- BTC-ETH 日回报相关性: **0.792**（2018+），高但非完美正相关
- 回测期: 2018-01-02 → 2026-02-18（~8 年）

### 策略对比（2018+ 数据）

| 策略 | CAGR | MaxDD | Sharpe | Calmar | WF 50% | WF 60% | Score |
|------|------|-------|--------|--------|--------|--------|-------|
| **BestCrypto** ⭐⭐⭐ | **149.2%** | -44.4% | **1.85** | **3.36** | ✅ 0.73 | ✅ 1.05 | **2.281** |
| BestCrypto (含成本) | 141.5% | -44.4% | 1.79 | 3.19 | — | — | 2.190 |
| CryptoDualMom | 123.6% | -42.0% | 1.67 | 2.94 | ✅ 0.77 | ✅ 1.09 | 2.046 |
| BTC v7f (baseline) | 84.8% | -42.6% | 1.49 | 1.99 | ✅ 1.07 | ✅ 1.58 | 1.564 |
| BTC B&H | 30.9% | -80.8% | 0.68 | 0.38 | — | — | 0.488 |
| ETH B&H | 15.7% | -93.5% | 0.61 | 0.17 | — | — | 0.343 |

### BestCrypto 策略逻辑

极其简洁的相对强度轮动：

1. 计算 BTC、ETH、GLD 各自 90 日动量
2. 选出动量最高的资产：
   - BTC 最强且 > 0: 70% BTC + 15% ETH + 10% GLD
   - ETH 最强且 > 0: 15% BTC + 65% ETH + 10% GLD
   - GLD 最强: 15% BTC + 10% ETH + 55% GLD
   - 最强资产动量 < 0: 15% BTC + 10% ETH + 35% GLD（40% 现金）

### 参数稳健性 ✅✅✅

**Lookback 敏感度**（所有值均通过 WF）：

| Lookback | CAGR | Sharpe | Calmar | WF 60% | Score |
|----------|------|--------|--------|--------|-------|
| 30d | 197.8% | 2.15 | 3.72 | ✅ 1.55 | 2.547 |
| 60d | 147.6% | 1.82 | 3.36 | ✅ 1.35 | 2.270 |
| **90d** | **149.2%** | **1.85** | **3.36** | ✅ 1.05 | 2.281 |
| 120d | 108.9% | 1.52 | 2.38 | ✅ 1.05 | 1.757 |
| 150d | 88.3% | 1.32 | 1.93 | ✅ 0.92 | 1.474 |
| 180d | 116.8% | 1.56 | 2.55 | ✅ 1.24 | 1.842 |

**Score Spread**: 2.547 - 1.474 = 1.073（较宽，30d 最好但可能过拟合，90d 是稳健选择）

**分配权重敏感度**（CryptoDualMom）：

| 强势权重 | CAGR | Sharpe | Calmar | Score |
|---------|------|--------|--------|-------|
| 0.45 | 116.6% | 1.61 | 2.73 | 1.934 |
| 0.55 | 129.3% | 1.68 | 2.98 | 2.064 |
| 0.65 | 142.3% | 1.75 | 3.21 | 2.182 |
| 0.75 | 155.4% | 1.80 | 3.43 | 2.292 |

所有参数下 Score 均 > 1.4，非常稳健。

### 逐年表现（BestCrypto 90d）

| Year | BTC B&H | ETH B&H | BestCrypto | 说明 |
|------|---------|---------|------------|------|
| 2018 | -73% | -82% | -12.1% | ⭐ GLD 保护 |
| 2019 | +95% | -2% | +71.1% | BTC 强势 |
| 2020 | +305% | +469% | +422.1% | ⭐ ETH DeFi 爆发 |
| 2021 | +60% | +399% | +270.8% | ⭐ ETH NFT 牛市 |
| 2022 | -65% | -67% | -16.5% | ⭐ GLD 保护 |
| 2023 | +156% | +91% | +116.3% | BTC 强势 |
| 2024 | +121% | +46% | +102.5% | BTC 强势 |
| 2025 | +10% | -31% | +21.1% | GLD 保护 |

### 关键发现

1. **ETH 提供了巨大的周期性 alpha**：2020-2021 ETH 远超 BTC（DeFi/NFT），轮动策略成功捕捉
2. **GLD 在两次加密寒冬中有效保护**：2018 和 2022 回撤仅 -12%~-16%（vs B&H -65%~-82%）
3. **Walk-Forward 全面通过**：所有参数组合在 50% 和 60% 分割下均通过 WF
4. **BTC-ETH 相关性 0.79 不影响策略**：因为策略利用的是相对强度差异，不是绝对分散化
5. **交易成本影响有限**：含 0.15%/side 成本后 CAGR 仅下降 8pp（149%→141%）

### ⚠️ 风险与注意事项

1. **数据仅 8 年**（2018-2025），样本量偏少
2. **加密货币市场结构变化快**：2018 的 ICO 时代 vs 2024 的 ETF 时代完全不同
3. **ETH 未来可能不再与 BTC 有显著差异**：如果 ETH 完全跟随 BTC，策略退化为 BTC v7f
4. **高换手率**（年化 2099%）：虽然加密货币手续费低，但频繁交易仍有执行风险
5. **需要同时持有 BTC、ETH、GLD 三种资产**：操作复杂度增加

### Composite Score 全局排行榜更新

| Rank | 策略 | CAGR | MaxDD | Sharpe | Calmar | Score | WF |
|------|------|------|-------|--------|--------|-------|-----|
| 🥇 | **BestCrypto (BTC+ETH+GLD)** ⭐⭐⭐ | 149.2% | -44.4% | 1.85 | 3.36 | **2.281** | ✅ |
| 🥈 | CryptoDualMom | 123.6% | -42.0% | 1.67 | 2.94 | 2.046 | ✅ |
| 🥉 | BTC v7f DualMom | 58.8% | -35.7% | 1.35 | 1.64 | 0.987 | ❌(17+) |
| 4 | Stock v4d DD+GLD | 27.1% | -15.0% | 1.45 | 1.81 | 1.356 | ✅ |
| 5 | Stock v3b SecRot | 25.8% | -17.7% | 1.35 | 1.46 | 1.173 | ✅ |
| 6 | TQQQ v9g GLD | 47.0% | -62.6% | 0.95 | 0.75 | 0.774 | ✅ |

**注意**：BestCrypto 和 CryptoDualMom 的回测期较短（2018-2025, 8年），与其他策略（10-15年）不完全可比。

### 失败方向

**UPRO Beast (3x S&P500)**：将 TQQQ v8/v9g 逻辑移植到 UPRO，结果全面失败。
- 所有变种 Sharpe 0.29-0.39，CAGR 8-12%，远低于 UPRO B&H（31%）
- 原因：S&P500 波动率低，趋势平滑，SMA200 滞后带产生大量假信号
- 结论：Beast 逻辑不适合低波动资产

### 文件
- `btc/codebear/crypto_dual_momentum.py` — BTC+ETH+GLD 轮动策略
- `leveraged_etf/codebear/upro_beast_v1.py` — UPRO Beast（失败）
- `portfolio/codebear/portfolio_v3_v4d_btc.py` — Portfolio v3 组合分析
- `data_cache/ETH_USD.csv` — ETH 日线数据

---
*Updated by 代码熊 🐻 — 2026-02-20 (Crypto Dual Momentum + UPRO Beast)*


---

## Portfolio v3: Stock v4d + BestCrypto 组合 (2026-02-20)

> 代码熊 🐻 | 将两个最优策略系列组合

### 核心发现：相关性仅 0.203！

Stock v4d（美股行业动量+GLD对冲）与 BestCrypto（BTC+ETH+GLD轮动）日回报相关性仅 0.203，比 BTC v7f 与 Stock 的 0.159 略高但仍非常低。

### 策略对比（2018-01 → 2025-12, ~8 年）

| 策略 | CAGR | MaxDD | Sharpe | Calmar | IS Sh | OOS Sh | WF60 | Score |
|------|------|-------|--------|--------|-------|--------|------|-------|
| Stock v4d Solo | 28.1% | -25.5% | 1.12 | 1.10 | 1.17 | 1.03 | 0.88 ✅ | 0.945 |
| BestCrypto Solo | 51.6% | -52.5% | 1.06 | 0.98 | 1.07 | 1.07 | 0.99 ✅ | 0.919 |
| Fixed 65/35 | 39.1% | -27.3% | 1.39 | 1.44 | — | — | 0.90 ✅ | **1.210** |
| Fixed 70/30 | 37.7% | -27.0% | 1.40 | 1.40 | 1.46 | 1.30 | 0.89 ✅ | 1.195 |
| **DD_Resp 126d** ⭐ | **33.0%** | **-22.2%** | **1.34** | **1.49** | 1.39 | 1.27 | **0.92 ✅** | **1.198** |
| RiskParity 252d | 42.1% | -26.8% | 1.48 | 1.57 | — | — | 0.85 ✅ | 1.304 |
| RiskParity 63d | 37.7% | -27.9% | 1.40 | 1.35 | 1.39 | 1.42 | 1.03 ✅ | 1.175 |

**所有组合方式均通过 WF 60/40！** ✅✅✅

### 参数稳健性 ✅✅✅

| 参数 | 测试范围 | Score 范围 | Spread |
|------|---------|-----------|--------|
| Fixed weight (stock%) | 30-80% | 1.031-1.210 | 0.179 |
| RP lookback | 21-252d | 1.012-1.304 | 0.292 |
| DD thresholds | 8组 | 1.153-1.198 | **0.045** ⭐ |
| BestCrypto lookback | 30-180d | 1.012-1.195 | 0.183 |

**DD Responsive 参数 spread 仅 0.045** — 极其稳健，几乎不受参数选择影响。

### 最优策略选择

**高收益导向: RiskParity 252d**
- CAGR 42.1%, Sharpe 1.48, Calmar 1.57, Score 1.304
- 长窗口风险平价让 crypto 在牛市获得更稳定的高权重

**最稳健/低回撤: DD_Resp 126d**
- CAGR 33.0%, MaxDD -22.2%, Calmar 1.49, WF60 0.92
- 组合回撤>5%时自动减仓，最大保护

**最佳平衡: Fixed 65/35 (65%Stock + 35%Crypto)**
- CAGR 39.1%, MaxDD -27.3%, Sharpe 1.39, Calmar 1.44, Score 1.210
- 最简单的实现，无需动态调整

### 逐年表现 (DD_Resp 126d)

| 年份 | 组合 | Stock v4d | BestCrypto |
|------|------|-----------|------------|
| 2018 | -4.6% | -0.0% | -12.9% |
| 2019 | 38.2% | 34.4% | 51.0% |
| 2020 | 75.7% | 47.6% | 152.0% |
| 2021 | 46.4% | 30.9% | 113.3% |
| 2022 | -10.5% | 0.0% | -43.7% |
| 2023 | 24.7% | 20.3% | 56.7% |
| 2024 | 36.6% | 31.3% | 48.2% |
| 2025 | 18.3% | 21.0% | 18.9% |
...
| 指标 | Stock Solo | Crypto Solo | 组合 (Fixed 65/35) |
|------|-----------|-------------|-------------------|
| Sharpe | 1.12 | 1.06 | **1.39** (+25%) |
| MaxDD | -25.5% | -52.5% | **-27.3%** |
| Calmar | 1.10 | 0.98 | **1.44** (+31%) |

两个 Sharpe ~1.1 的策略，因低相关性组合后 Sharpe 提升至 1.39。

### Composite Score 全局排行榜更新

| Rank | 策略 | CAGR | MaxDD | Sharpe | Calmar | Score | WF | 数据期 |
|------|------|------|-------|--------|--------|-------|-----|--------|
| 🥇 | **BestCrypto** | 149.2% | -44.4% | 1.85 | 3.36 | **2.281** | ✅ | 8y |
| 🥈 | Stock v4d | 27.1% | -15.0% | 1.45 | 1.81 | **1.356** | ✅ | 11y |
| 🥉 | **Portfolio v3 RP252** | 42.1% | -26.8% | 1.48 | 1.57 | **1.304** | ✅ | 8y |
| 4 | **Portfolio v3 Fixed65** | 39.1% | -27.3% | 1.39 | 1.44 | **1.210** | ✅ | 8y |
| 5 | **Portfolio v3 DDResp** | 33.0% | -22.2% | 1.34 | 1.49 | **1.198** | ✅ | 8y |
| 6 | Stock v3b | 25.8% | -17.7% | 1.35 | 1.46 | 1.173 | ✅ | 11y |
| 7 | BTC v7f | 58.8% | -35.7% | 1.35 | 1.64 | 0.987 | ❌ | 9y |
| 8 | TQQQ v9g | 47.0% | -62.6% | 0.95 | 0.75 | 0.774 | ✅ | 16y |

### 文件
- `portfolio/codebear/portfolio_v3_stock_crypto.py` — 全期组合分析
- `portfolio/codebear/portfolio_v3_2018plus.py` — 2018+ 17种组合方式对比
- `portfolio/codebear/portfolio_v3_robustness.py` — 参数稳健性测试

---
*Updated by 代码熊 🐻 — 2026-02-20 (Portfolio v3: Stock v4d + BestCrypto)*

---

## BestCrypto 稳健性测试（2026-02-20）

**测试目的**：验证 BestCrypto 是否过度依赖 ETH 某一年的超级大涨。

**结果**：ETH 最佳年份为 2020 (+481.0%)。剔除该年 ETH 后：
- CAGR: 149.2% → 123.4% (Δ -25.7%)
- Sharpe: 1.85 → 1.71 (仍 > 1.5 ✅)
- Composite: 2.281 → 1.994 (仍远超 Stock v4d 1.356 ✅)

即使同时剔除 2020+2021 两年的 ETH，Composite 仍 = 1.636，超过所有基准。

**结论：✅ BestCrypto 策略稳健，不依赖单一年份的极端表现。**

详见 `btc/codebear/bestcrypto_robustness_report.md`
