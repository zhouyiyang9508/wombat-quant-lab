#!/usr/bin/env python3
"""
动量轮动 v9j Final — TLT条件国债对冲 (熊市+TLT动量正)
代码熊 🐻

📊 性能摘要 (2015-2025):
  目标: Composite > 2.0, WF > 0.75

v9i创新栈 (13层) + 第14层: 条件TLT熊市对冲

核心发现 (来自v9j参数扫描):
  - TLT自然竞争: 无显著贡献 (thresh=9.99最优=关闭)
  - TLT熊市替代: 将熊市现金的15%换成TLT对冲 → Composite 2.04

关键风险:
  - 2022年加息周期: SPY跌20%, TLT同步跌30%! (股债双杀)
  - 普通TLT熊市配置WF从0.82降至0.74

v9j Final创新:
  - 条件TLT: 只在 (熊市 AND TLT_6m_mom > 0) 时配置TLT
  - 2022年: SPY下跌 + TLT动量为负 → TLT条件不满足 → 保留SHY
  - 2018/2020: SPY下跌 + TLT动量为正 → 成功配置TLT
  - 这样避免股债双杀, 保持WF稳健性

严格无前瞻: 使用月末前一个交易日的TLT价格计算动量
"""
...
# v9j Final: Conditional TLT Bear Hedge
TLT_BEAR_FRAC = 0.15     # 15% TLT in bear mode (when TLT momentum > 0)
TLT_MOM_LOOKBACK = 126   # 6-month momentum for TLT condition check
...
def get_tlt_momentum(tlt_p, date):
    """Check if TLT has positive 6m momentum (no lookahead)"""
...
def select(sig, sectors, date, prev_hold, gld_p, gdx_p, tlt_p):
    ...
    # Conditional TLT: only in bear mode AND TLT has positive momentum
    ...
    if reg == 'bull':
        ...
        sps = BEAR_SPS
        # If TLT condition met: reduce SHY cash by TLT_BEAR_FRAC
...
def run_backtest(close_df, sig, sectors, gld_p, gdx_p, gdxj_p, shy_p, tlt_p,
    ...
    return eq, holdings_hist, avg_to, avg_scale, tlt_months
...
def main():
    print("=" * 72)
    print("🐻 动量轮动 v9j Final — 条件TLT熊市对冲 (bear+TLT_mom>0)")
    print("=" * 72)
    global TLT_BEAR_FRAC
    print(f"\nConfig (v9i + conditional TLT bear):")
    print(f"  TLT Bear Frac: {TLT_BEAR_FRAC:.0%} (only when TLT 6m momentum > 0)")
    print(f"  TLT Mom Lookback: {TLT_MOM_LOOKBACK} days")
    print(f"  Vol Target: {VOL_TARGET_ANN:.0%}/yr (same as v9i)")
    ...
    # Quick sweep of TLT_BEAR_FRAC
    print("\n🔍 Sweeping TLT bear fraction...")
    sweep = []
    for frac in [0.10, 0.12, 0.15, 0.18, 0.20, 0.25]:
        TLT_BEAR_FRAC = frac
        eq, _, to, _, tlt_m = run_backtest(close_df, sig, sectors, gld_p, gdx_p, gdxj_p, shy_p, tlt_p)
        ...
        sweep.append({'frac': frac, 'composite': comp, 'sharpe': m['sharpe'],
                      'cagr': m['cagr'], 'max_dd': m['max_dd'], 'calmar': m['calmar'],
                      'wf': wf, 'tlt_months': tlt_m})
        print(f"  TLT_bear={frac:.2f} → Comp={comp:.4f} Sharpe={m['sharpe']:.2f} "
              f"MaxDD={m['max_dd']:.1%} WF={wf:.2f} TLT_months={tlt_m}")

    best = max(sweep, key=lambda x: x['composite'])
    TLT_BEAR_FRAC = best['frac']
    print(f"\n🏆 Best TLT bear frac: {TLT_BEAR_FRAC}")

    print("\n🔄 Full (2015-2025)...")
    eq_full, hold, to, avg_scale, tlt_months = run_backtest(
    ...
    print("\n" + "=" * 72)
    print("📊 RESULTS — v9j Final vs v9i Champion")
    print("=" * 72)
    v9i = dict(cagr=0.319, max_dd=-0.107, sharpe=1.81, calmar=2.97, comp=1.973, wf=0.82)
    print(f"{'Metric':<14} {'v9i':<10} {'v9j_final':<12} {'Delta'}")
    print(f"{'CAGR':<14} {v9i['cagr']:.1%}     {m['cagr']:.1%}       {m['cagr']-v9i['cagr']:+.1%}")
    print(f"{'MaxDD':<14} {v9i['max_dd']:.1%}    {m['max_dd']:.1%}      {m['max_dd']-v9i['max_dd']:+.1%}")
    print(f"{'Sharpe':<14} {v9i['sharpe']:.2f}      {m['sharpe']:.2f}        {m['sharpe']-v9i['sharpe']:+.2f}")
    print(f"{'Calmar':<14} {v9i['calmar']:.2f}      {m['calmar']:.2f}        {m['calmar']-v9i['calmar']:+.2f}")
    print(f"{'IS Sharpe':<14} -         {mi['sharpe']:.2f}")
    print(f"{'OOS Sharpe':<14} -         {mo['sharpe']:.2f}")
    print(f"{'WF':<14} {v9i['wf']:.2f}      {wf:.2f}        {wf-v9i['wf']:+.2f}")
    print(f"{'Composite':<14} {v9i['comp']:.4f}  {comp:.4f}    {comp-v9i['comp']:+.4f}")
    print(f"\n📅 TLT conditional bear: {tlt_months}/{len(hold)} months")

    print("\n📊 Parameter sweep summary:")
    for r in sweep:
        print(f"  frac={r['frac']:.2f} Comp={r['composite']:.4f} Sharpe={r['sharpe']:.2f} "
              f"MaxDD={r['max_dd']:.1%} WF={r['wf']:.2f} TLT={r['tlt_months']}m")

    if comp > 2.0:
        print(f"\n🚨🚨🚨 【重大突破】Composite > 2.0! ({comp:.4f})")
        if wf >= 0.80:
            print(f"🚨🚨🚨 AND WF >= 0.80! ({wf:.2f}) — EXTREMELY ROBUST!")
    elif comp > 1.973:
    ...
    out = {
        'strategy': f'v9j_final Conditional TLT Bear Hedge (bear_frac={TLT_BEAR_FRAC}, tlt_mom6m>0)',
        'full': m, 'is': mi, 'oos': mo,
        'wf': float(wf), 'composite': float(comp), 'turnover': float(to),
        'tlt_months': tlt_months, 'total_months': len(hold),
        'sweep': sweep,
        'params': {
            'tlt_bear_frac': TLT_BEAR_FRAC,
            'tlt_mom_lookback': TLT_MOM_LOOKBACK,
            'vol_target': VOL_TARGET_ANN,
            'vol_lookback': VOL_LOOKBACK,
        }
    }
    jf = Path(__file__).parent / "momentum_v9j_final_results.json"
