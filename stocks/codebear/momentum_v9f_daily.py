#!/usr/bin/env python3
"""
动量轮动 v9f-Daily — 日频净值追踪版本
代码熊 🐻

基于 v9f_final 的选股逻辑，改为月频调仓 + 日频净值追踪
修复原始月频回测中 MaxDD 严重低估的缺陷

📊 性能对比 (2015-2025):
               月频(原始)    日频(真实)
  CAGR:        34.59%       35.39%
  MaxDD:       -14.89%      -26.51% ← 真实值！低估1.78倍
  Sharpe:      1.67         1.35
  Calmar:      2.32         1.34
  Composite:   1.6671       1.1464
...
def main():
    ...
    v9f_mod = load_strategy_module("v9f", Path(__file__).parent / "momentum_v9f_final.py")
    sig = v9f_mod.precompute(close_df)

    # A. 无止损
    print("\n🔄 日频回测 (无止损)...")
    eq_A = run_daily_backtest(close_df, sig, sectors, gld_p, gdx_p, gdxj_p, shy_p,
                              v9f_mod.select, v9f_mod.apply_overlays, v9f_mod.get_spy_vol)
    ...
    out = {
        'strategy': 'v9f-Daily (日频净值追踪)',
        'no_stop': m_A, 'sl15': m_B, 'sl10': m_C,
        'is': m_IS, 'oos': m_OOS, 'wf': float(wf),
    }
    jf = Path(__file__).parent / "momentum_v9f_daily_results.json"
