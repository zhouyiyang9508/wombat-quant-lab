#!/usr/bin/env python3
"""
v11b Master 日频审计 — 真实 MaxDD 验证
代码熊 🐻 | 2026-02-22

方法（与 daily_audit_v10d.py 完全一致）：
  1. 导入 v11b 模块，获取月频持仓逻辑
  2. 构建所有资产的日频收益矩阵
  3. 月频调仓，日频追踪净值
  4. 计算真实日频 MaxDD
...
spec = importlib.util.spec_from_file_location(
    "v11b", Path(__file__).parent / "momentum_v11b_final.py")
...
def main():
    print("=" * 72)
    print("🔍 v11b Master 日频审计 — 真实 MaxDD 验证")
    print("=" * 72)
    ...
    HEDGE_KEYS = v11b.HEDGE_KEYS  # {'GLD', 'GDX', 'GDXJ', 'TLT', 'IEF', 'XLV', 'XLP', 'XLU'}

    # ── Step 1: 月频调仓，记录每期持仓 ──────────────────────────────────────
    print("  Computing monthly rebalances (v11b logic)...")
    ...
    monthly_reported = -0.094  # v11b 月频报告值

    print("\n" + "=" * 72)
    print("📊 v11b Master 日频审计结果")
    print("=" * 72)
    ...
    print(f"    v11b:    月频{monthly_reported:.1%} → 日频{max_dd:.1%}  ({abs(max_dd)/abs(monthly_reported):.2f}x)")
    ...
    jf = Path(__file__).parent / "daily_audit_v11b_results.json"
